...
export function StepExport({
  ...
  async function handleExport() {
    ...
    try {
      ...
      triggerDownload(await res.blob(), `envosta-child-${slug}.zip`);

      // 2) WXR content XML (built client-side from the shared studio-wxr lib)
      ...
      <div className="max-w-2xl w-full space-y-6">
        ...
        <div className="rounded-xl border border-gray-200 bg-gray-50/50 p-4">
          ...
          <p className="text-[11px] text-gray-500 mt-1.5 leading-snug">
            Folder name of the parent theme inside <code className="bg-white px-1 rounded">wp-content/themes/</code>. Default is <code className="bg-white px-1 rounded">Envosta-wordpress-theme</code> (case-sensitive, matches the <a className="underline" href="https://github.com/GetEnvosta/Envosta-wordpress-theme" target="_blank" rel="noopener">Envosta parent theme repo</a>). Change to <code className="bg-white px-1 rounded">assembler</code> to run on stock Automattic Assembler.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <div className="bg-gray-50 rounded-xl border border-gray-200 p-4 font-mono text-xs text-gray-600">
            ...
            <p className="mt-3 text-[11px] text-gray-500 font-sans leading-snug">
              Child theme that targets <code className="bg-white px-1 rounded">Envosta-wordpress-theme</code>. Minimal when Full Custom is off; contains color / font / radius overrides when on.
            </p>
          </div>
          {/* 2. WXR content */}
          <div className="bg-gray-50 rounded-xl border border-gray-200 p-4 font-mono text-xs text-gray-600">
            ...
            <p className="ml-5 text-[11px] text-gray-500 font-sans leading-snug">
              WXR with {pagesWithContent.length} page{pagesWithContent.length === 1 ? '' : 's'} + a Main Menu. Import via <strong>Tools → Import → WordPress</strong> after the child theme is active.
            </p>
          </div>
        </div>
        ...
        <div className="bg-blue-50 rounded-xl border border-blue-200 p-5">
          ...
          <ol className="text-xs text-blue-800 space-y-2 list-decimal list-inside">
            <li>Confirm the <strong>Envosta parent theme</strong> (<code className="bg-blue-100 px-1 rounded">Envosta-wordpress-theme</code>) is already installed — it ships with every Envosta-hosted site by default.</li>
