...
export function StepExport({
  ...
  async function handleExport() {
    ...
    try {
      ...
      // Always ship the child theme. Even in Parent mode it's a tiny stub
      // whose functions.php runs a one-shot setup on activation to apply
      // site title, tagline, static homepage, posts page, and the Main Menu
      // location — so the user doesn't have to touch Settings → Reading
  ...
  return (
    <div className="flex items-start justify-center min-h-full p-8">
      <div className="max-w-2xl w-full space-y-6">
        ...
        {/* Two downloads — child theme (with setup hook) + WXR */}
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="bg-gray-50 rounded-xl border border-gray-200 p-4 font-mono text-xs text-gray-600">
            ...
            <p className="mt-3 text-[11px] text-gray-500 font-sans leading-snug">
              {fullCustom
                ? 'Child theme with your Full Custom palette / fonts / radius overrides + a one-shot setup hook that sets site title, tagline, homepage, posts page, and Main Menu location after activation.'
                : 'Child theme (minimal, inherits all styles from the parent) + a one-shot setup hook that sets site title, tagline, homepage, posts page, and Main Menu location after activation.'}
            </p>
          </div>
          <div className="bg-gray-50 rounded-xl border border-gray-200 p-4 font-mono text-xs text-gray-600">
            <div className="flex items-center gap-2 text-gray-900 font-semibold mb-2">
            ...
            <p className="ml-5 text-[11px] text-gray-500 font-sans leading-snug">
              WXR with {pagesWithContent.length} page{pagesWithContent.length === 1 ? '' : 's'} as real Gutenberg blocks + a Main Menu linking them. Import via <strong>Tools → Import → WordPress</strong> after activating the child theme.
            </p>
          </div>
        </div>
        ...
        <div className="bg-blue-50 rounded-xl border border-blue-200 p-5">
          ...
          <ol className="text-xs text-blue-800 space-y-2 list-decimal list-inside">
            <li>Confirm the <strong>Envosta parent theme</strong> (<code className="bg-blue-100 px-1 rounded">Envosta-wordpress-theme</code>) is installed — it ships with every Envosta-hosted site by default.</li>
            ...
            <li>Upload <code className="bg-blue-100 px-1 rounded">envosta-child-{slug}.zip</code> via <strong>Appearance → Themes → Add New → Upload Theme</strong>, then <strong>Activate</strong> it. The activation hook automatically sets site title / tagline / static homepage / posts page / Main Menu location.</li>
            {!fullCustom && styleConfig?.presetId && (
              <li>Optional: go to <strong>Appearance → Editor → Styles</strong> and apply the <strong>"{String(styleConfig.presetId).replace(/^assembler-/, '')}"</strong> style variation for the designed palette + typography.</li>
            )}
            <li>Done — no manual trips to Settings → Reading or Appearance → Menus needed.</li>
          </ol>
        </div>
      </div>
    </div>
