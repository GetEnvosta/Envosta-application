...
You're welcome to customise the attributes: overlayMenu can be "mobile"|"always"|"never"; icon can be "menu"|"menu-alt"|"menu-alt2"|"menu-alt3"; overlayBackgroundColor/overlayTextColor accept any theme-N slug; add hasIcon:true for a branded trigger; openSubmenusOnClick:true makes nested menus tap-to-open. You can wrap the Navigation block in a <div> with your own styles for position, padding, background, and motion — just keep the <!-- wp:navigation /--> comment intact so WordPress renders the real mobile slide-out menu on import. If WooCommerce is on, also include a styled cart icon that links to /cart and an account icon linking to /my-account alongside the nav.`, html: '' });
  ...
  // Sections panel collapse state — default closed. Editing state holds the
  // id of the section currently being edited inline + the draft values.
  ...
  return (
    <div className="flex h-full">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Sections panel — collapsible, default closed. Sits directly above
            the prompt bar so it reads as part of the same compose area. */}
        {selectedPage && (() => {
          const sections = Array.isArray(selectedPage.sections) ? selectedPage.sections : [];
          const sectionCount = sections.length;

          function commitSections(next: any[]) {
            onPagesChange(pages.map(p => p.id === selectedPage.id ? { ...p, sections: next } : p));
          }
          function startEdit(s: any) {
            setEditingSectionId(s.id);
            setEditDraft({ title: s.title || '', description: s.description || '' });
          }
          function cancelEdit() {
            setEditingSectionId(null);
            setEditDraft({ title: '', description: '' });
          }
          function saveEdit(sid: string) {
            const title = editDraft.title.trim();
            if (!title) { cancelEdit(); return; }
            const next = sections.map((x: any) => x.id === sid
              ? { ...x, title, description: editDraft.description.trim() }
              : x);
            commitSections(next);
            cancelEdit();
          }
          function addSection() {
            const title = window.prompt('Section title (e.g. "Testimonials"):', '')?.trim();
            if (!title) return;
            const description = window.prompt('One-sentence description:', '')?.trim() || '';
            const id = title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || `section-${Date.now()}`;
            commitSections([...sections, { id, title, description }]);
            setStatus({ type: 'success', msg: `Added "${title}" — regenerate the page to render it` });
            setSectionsOpen(true);
          }

          return (
            <div className="border-t border-gray-200 bg-gray-50/60 shrink-0">
              {/* Collapsible header */}
              <button
                onClick={() => setSectionsOpen(!sectionsOpen)}
                className="w-full flex items-center gap-2 px-4 py-2 hover:bg-gray-100 transition-colors"
              >
                {sectionsOpen ? <ChevronUp className="w-3 h-3 text-gray-400" /> : <ChevronDown className="w-3 h-3 text-gray-400" />}
                <span className="text-[10px] font-semibold text-gray-500 uppercase tracking-widest">
                  Sections in {selectedPage.title}
                </span>
                <span className="text-[10px] text-gray-400">
                  {sectionCount === 0 ? 'none yet' : `${sectionCount}`}
                </span>
                {regenKey?.startsWith(`${selectedPage.id}:`) && (
              ...
              {sectionsOpen && (
                <div className="px-4 pb-3 max-h-[240px] overflow-y-auto">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <button
                      onClick={async () => {
                        if (sectionCount > 0 && !confirm(`Re-plan sections for "${selectedPage.title}"? This replaces the current section list. The next generation will use the new plan.`)) return;
                        const planned = await planSections(selectedPage.id);
                        if (planned && planned.length > 0) {
                          commitSections(planned);
                    ...
                    >
                      {sectionCount === 0 ? 'AI plan' : 'Re-plan'}
                    </button>
                    <span className="text-[10px] text-gray-400 leading-snug">
                      Edit the plan, then hit <strong>Send</strong> below to regenerate against it.
                    </span>
                  </div>

                  {sectionCount === 0 ? (
                    <p className="text-[11px] text-gray-500 leading-snug py-2">
                      No sections planned yet. Click <strong>AI plan</strong> to have Claude draft them, or <strong>+ Add section</strong> to build your own.
                    </p>
                  ) : (
                      ...
                      {sections.map((s: any, i: number) => {
                        ...
                        return (
                          <li key={s.id} className="group flex items-start gap-2">
                            <span className="text-gray-400 font-mono shrink-0 mt-1">{String(i + 1).padStart(2, '0')}</span>
                            {isEditing ? (
                              <div className="flex-1 min-w-0 space-y-1">
                                <input
                                  ...
                                  className="w-full rounded border border-indigo-300 px-2 py-1 text-[11px] font-medium text-gray-800 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-200 outline-none"
                                ...
                                <input
                                  ...
                                  onKeyDown={e => { if (e.key === 'Enter') saveEdit(s.id); if (e.key === 'Escape') cancelEdit(); }}
                                  className="w-full rounded border border-indigo-300 px-2 py-1 text-[11px] text-gray-600 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-200 outline-none"
                                  placeholder="One-sentence description"
                                />
                                <div className="flex items-center gap-2">
                                  <button onClick={() => saveEdit(s.id)} className="text-[10px] font-medium text-indigo-600 hover:text-indigo-800">Save</button>
                                  <button onClick={cancelEdit} className="text-[10px] text-gray-500 hover:text-gray-700">Cancel</button>
                                </div>
                              </div>
                            ) : (
                              <button
                                type="button"
                                onClick={() => startEdit(s)}
                                className="flex-1 min-w-0 text-left rounded hover:bg-white/60 px-1 -mx-1 py-0.5 transition-colors"
                              ...
                              <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity mt-0.5">
                                ...
                                <button
                                  onClick={async () => {
                                    ...
                                    if (!instr) return;
                                    const next = sections.map((x: any) => x.id === s.id ? { ...x, description: instr } : x);
                                    commitSections(next);
                                    regenerateSection(selectedPage.id, s.id, instr);
                                  }}
                                  disabled={!!regenKey}
                                  className="p-0.5 text-gray-400 hover:text-indigo-600 disabled:opacity-30"
                                  title="Prompt-drive a regeneration"
                                ...
                                <button
                                  onClick={() => {
                                    ...
                                    const next = sections.filter((x: any) => x.id !== s.id);
                                    commitSections(next);
        ...
        <div className="border-t border-gray-200 bg-white px-4 py-3 shrink-0">
          ...
          <div className="flex items-center gap-2 w-full">
            ...
            <input
              ...
              className="flex-1 min-w-0 rounded-lg border border-gray-200 px-3 py-2 text-sm text-gray-900 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 outline-none"
