...
import { createSite, mapDomain, confirmSsl, wpcloudNameservers } from "@/spine/wpcloud";
...
function supabaseClient(): SupabaseClient {
  ...
  if (!url || !key) throw new Error("Supabase env missing for spine/provisioning");
...
export async function enqueueProvisioning(clientId: string) {
  ...
  const { data, error } = await supabaseClient().from("provisioning_jobs")
...
export async function runJob(jobId: string) {
  const { data: job, error } = await supabaseClient().from("provisioning_jobs").select("*").eq("id", jobId).single();
  if (error || !job) throw new Error(`Job not found: ${jobId}`);
  const steps: StepRecord[] = (job.steps as StepRecord[]) ?? [];

  const { data: client } = await supabaseClient().from("clients").select("*").eq("id", job.client_id).single();
  if (!client) throw new Error("Client missing for job");

  await supabaseClient().from("provisioning_jobs").update({ status: "running", attempts: job.attempts + 1 }).eq("id", jobId);

  const record = async (step: Step, status: StepRecord["status"], detail?: string) => {
    steps.push({ step, status, at: new Date().toISOString(), detail });
    await supabaseClient().from("provisioning_jobs").update({ steps, current_step: step }).eq("id", jobId);
  };

  try {
    // ---- 1. register_domain -------------------------------------------------
    if (!stepDone(steps, "register_domain")) {
      let { data: domain } = await supabaseClient().from("domains").select("*").eq("client_id", client.id).maybeSingle();
      if (!domain) {
        const { data: created, error: dErr } = await supabaseClient().from("domains").insert({
          client_id: client.id,
      ...
      if (!job.dry_run && !domain.registrant) {
        ...
        await supabaseClient().from("provisioning_jobs").update({ status: "blocked", error: "Missing registrant details" }).eq("id", jobId);
        return;
      }
    ...
    if (!stepDone(steps, "configure_dns")) {
      const { data: domain } = await supabaseClient().from("domains").select("*").eq("client_id", client.id).single();
    ...
    if (!stepDone(steps, "create_site")) {
      const { data: domain } = await supabaseClient().from("domains").select("*").eq("client_id", client.id).single();
      let { data: site } = await supabaseClient().from("sites").select("*").eq("client_id", client.id).maybeSingle();
      if (!site) {
        const { data: created, error: sErr } = await supabaseClient().from("sites")
    ...
    if (!stepDone(steps, "map_domain")) {
      const { data: site } = await supabaseClient().from("sites").select("*").eq("client_id", client.id).single();
      const { data: domain } = await supabaseClient().from("domains").select("*").eq("client_id", client.id).single();
    ...
    for (const manual of ["studio_build", "gbp_setup", "monitoring"] as const) {
      if (!stepDone(steps, manual)) {
        await supabaseClient().from("deliverables").insert({
          client_id: client.id,
          type: "site_edit",
        ...
        await supabaseClient().from("provisioning_jobs").update({ status: "blocked" }).eq("id", jobId);
    ...
    if (!stepDone(steps, "go_live")) {
      await supabaseClient().from("sites").update({ status: "live" }).eq("client_id", client.id);
    ...
    await supabaseClient().from("provisioning_jobs").update({ status: "done", error: null }).eq("id", jobId);
    await supabaseClient().from("audit_log").insert({
      actor: "system", action: "provisioning.completed", entity: "client", entity_id: client.id, meta: { jobId },
    });
  } catch (err) {
    await supabaseClient().from("provisioning_jobs").update({ status: "failed", error: String(err) }).eq("id", jobId);
    throw err;
  }
}
...
export async function resumeJob(jobId: string, completedStep: Step) {
  const { data: job } = await supabaseClient().from("provisioning_jobs").select("*").eq("id", jobId).single();
  ...
  await supabaseClient().from("provisioning_jobs").update({ steps, status: "queued" }).eq("id", jobId);
  return runJob(jobId);
}

export async function runNextJob() {
  const { data } = await supabaseClient().from("provisioning_jobs")
