      ...
      <table className="w-full">
        <thead>
          <tr className="border-b border-gray-100">
            ...
            <th className="text-right text-xs font-medium text-gray-500 uppercase tracking-wider px-4 py-2.5">Credits/Unit</th>
          ...
          {Object.entries(grouped).map(([serviceType, rows]) => (
            rows.map((row, idx) => (
              <tr key={row.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                ...
                <td className="px-4 py-2.5 text-sm text-gray-700 font-mono text-xs">
                  {row.metric}
                </td>
                ...
                <td className="px-4 py-2.5 text-right">
                  {editingId === row.id ? (
                    <div className="flex items-center justify-end gap-1">
                      ...
                      />
                      <button
                        onClick={() => handleSave(row.id, { credits_per_unit: parseFloat(editValue) })}
                    ...
                    <button
                      onClick={() => { setEditingId(row.id); setEditValue(String(row.credits_per_unit)); }}
