...
export async function POST(req: Request) {
  ...
  if (!site.twilio_phone_number) return NextResponse.json({ error: 'Site has no phone number' }, { status: 400 });

  const phoneNumber = site.twilio_phone_number;
  ...
  try {
    ...
    await supabase.from('logs').insert({
      ...
      details: `Admin released ${phoneNumber} from site "${site.label}"`,
      level: 'info',
