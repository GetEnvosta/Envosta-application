...
export function SalesProgressBar({ currentStage }: { currentStage: string }) {
  const currentIdx = STAGES.findIndex(s => s.key === currentStage);

  return (
    <div className="card p-5 mb-6">
      <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-4">Sales Pipeline</h3>
...
export function SalesStageAdvancer({
  ticketId,
  currentStage,
}: {
  ticketId: string;
  ...
  async function advanceTo(targetStage: string) {
    ...
    const metadata = { ...(ticket?.metadata as any ?? {}), sales_stage: targetStage };
